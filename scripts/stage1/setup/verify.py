...
CHECKPOINT = os.path.join(REPO_ROOT, 'models', 'pretrained', 'CRNN_note_F1=0.9677_pedal_F1=0.9186.pth')
...
def main():
    ...
    check(
        f'Checkpoint size: {ckpt_size_mb:.0f}MB', 160 < ckpt_size_mb < 170, f'expected ~164MB, got {ckpt_size_mb:.0f}MB'
    )
    ...
    check(
        'Silent audio produces 0 notes',
        len(result['note_events']) == 0,
    ...
    loader = DataLoader(train_ds, batch_size=2, shuffle=True, num_workers=0, collate_fn=collate_fn, drop_last=True)
    ...
    check('Loss is finite', all(np.isfinite(loss) for loss in losses), f'losses={losses}')
