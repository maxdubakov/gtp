...
def print_sample_summary(idx, sample):
    ...
    for key in EXPECTED_KEYS:
        arr = sample[key]
        print(f'    {key:20s}  shape={arr.shape}  min={arr.min():.4f}  max={arr.max():.4f}  mean={arr.mean():.4f}')
...
def verify_shapes_and_ranges(sample, segment_samples, frames_num, classes_num):
    ...
    assert waveform.shape == (segment_samples,), f'waveform shape {waveform.shape} != ({segment_samples},)'
    ...
    for key in ['onset_roll', 'offset_roll', 'frame_roll']:
        ...
        assert arr.min() >= 0.0 and arr.max() <= 1.0, f'{key} out of [0,1]: min={arr.min()}, max={arr.max()}'
    ...
    assert vel.min() >= 0.0 and vel.max() <= 127.0, f'velocity_roll out of [0,127]: min={vel.min()}, max={vel.max()}'

    mask = sample['mask_roll']
    assert mask.shape == roll_shape, f'mask_roll shape {mask.shape} != {roll_shape}'
    assert set(np.unique(mask)).issubset({0.0, 1.0}), f'mask_roll has non-binary values: {np.unique(mask)}'
...
def show_verbose_trace(gaps_dir, guitarset_dir):
    ...
    if not os.path.exists(first_jams):
        first_jams = sorted(f for f in os.listdir(ann_dir) if f.startswith('05') and f.endswith('.jams'))
        if first_jams:
    ...
    def _show_roll_stats(source_label, notes):
        ...
        for key, arr in rolls.items():
            ...
            print(f'    {key:20s}  nonzero/non-default={nonzero:5d}  min={arr.min():.4f}  max={arr.max():.4f}')
...
def main():
    ...
    parser.add_argument('-v', '--verbose', action='store_true', help='Enable trace logging and show annotation detail')
    parser.add_argument('-n', type=int, default=4, help='Number of samples to load and inspect (default: 4)')
    ...
    frames_num = round(segment_seconds * frames_per_second) + 1
    classes_num = 88
    ...
    for key in ['onset_roll', 'reg_onset_roll', 'frame_roll', 'velocity_roll', 'mask_roll']:
        ...
        print(
            f'  {key:20s}  global min={vals.min():.4f}  max={vals.max():.4f}  '
